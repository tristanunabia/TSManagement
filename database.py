...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS instructors (
        ...
        academic_level TEXT DEFAULT 'Tertiary' CHECK(academic_level IN ('Tertiary', 'SHS'))
    )
    """)
    
    # Check if academic_level column exists in instructors (migration)
    cursor.execute("PRAGMA table_info(instructors)")
    columns = [col[1] for col in cursor.fetchall()]
    if "academic_level" not in columns:
        cursor.execute("ALTER TABLE instructors ADD COLUMN academic_level TEXT DEFAULT 'Tertiary'")
    
    # 2. Class Loads Table
    # Check if existing class_loads table has 'Excess Load' in its CHECK constraint
    cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='class_loads'")
    cl_table_row = cursor.fetchone()
    if cl_table_row is None:
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS class_loads (
            ...
            units REAL NOT NULL,
            hours REAL NOT NULL,
            load_category TEXT NOT NULL CHECK(load_category IN ('Regular Load', 'Excess Load', 'Excess/Overload', 'Consultation')),
            enrollment INTEGER DEFAULT 0,
            FOREIGN KEY(instructor_id) REFERENCES instructors(id) ON DELETE CASCADE
        )
        """)
    elif "'Excess Load'" not in cl_table_row['sql']:
        cursor.execute("PRAGMA foreign_keys=OFF")
        cursor.execute("""
        CREATE TABLE class_loads_new (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            instructor_id INTEGER NOT NULL,
            subject_name TEXT NOT NULL,
            section_code TEXT NOT NULL,
        ...
        """)
        cursor.execute("""
        INSERT INTO class_loads_new (id, instructor_id, subject_name, section_code, room, day_of_week, start_time, end_time, type, units, hours, load_category, enrollment)
        SELECT id, instructor_id, subject_name, section_code, room, day_of_week, start_time, end_time, type, units, hours,
               CASE WHEN load_category = 'Excess/Overload' THEN 'Excess Load' ELSE load_category END,
               enrollment
        FROM class_loads
        """)
        cursor.execute("DROP TABLE class_loads")
        cursor.execute("ALTER TABLE class_loads_new RENAME TO class_loads")
        cursor.execute("PRAGMA foreign_keys=ON")
    else:
        cursor.execute("UPDATE class_loads SET load_category = 'Excess Load' WHERE load_category = 'Excess/Overload'")
    ...
    if cursor.fetchone()['count'] == 0:
        # Prepopulate Jane Doe (SC-Based, Tertiary)
        cursor.execute("""
        INSERT INTO instructors (name, email, phone, department, employment_status, academic_level)
        VALUES ('Jane Doe', 'jane.doe@university.edu', '555-0192', 'Computer Science', 'SC-Based', 'Tertiary')
        """)
        ...
        classes = [
            (jane_id, 'Computer Programming 5', 'ICTE201P', 'Room 301', 'W', '12:30', '14:00', 'LEC', 2.0, 1.5, 'Regular Load', 0),
            (jane_id, 'Computer Programming 5', 'ICTE201P', 'Lab 3', 'F', '11:00', '14:00', 'LAB', 1.0, 3.0, 'Regular Load', 0),
            (jane_id, 'Data Structures', 'ICTE202P', 'Room 402', 'T', '09:00', '12:00', 'LEC', 3.0, 3.0, 'Regular Load', 0),
        ...
        # Prepopulate John Smith (Part-Time, SHS)
        cursor.execute("""
        INSERT INTO instructors (name, email, phone, department, employment_status, academic_level)
        VALUES ('John Smith', 'john.smith@university.edu', '555-0143', 'Information Technology', 'Part-Time', 'SHS')
        """)
        john_id = cursor.lastrowid
        
        # John Smith class loads
        john_classes = [
            (john_id, 'Web Development', 'ICTE302', 'Lab 5', 'TH', '13:00', '16:00', 'LAB', 2.0, 3.0, 'Excess Load', 0),
            (john_id, 'Database Systems', 'ICTE305', 'Room 204', 'M', '14:00', '17:00', 'LEC', 3.0, 3.0, 'Regular Load', 0),
            (john_id, 'Database Systems', 'ICTE305', 'Lab 1', 'W', '09:00', '12:00', 'LAB', 1.0, 3.0, 'Regular Load', 0)
        ]
        cursor.executemany("""
        INSERT INTO class_loads (instructor_id, subject_name, section_code, room, day_of_week, start_time, end_time, type, units, hours, load_category, enrollment)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
def add_instructor(name, email, phone, department, employment_status, academic_level="Tertiary"):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("""
    INSERT INTO instructors (name, email, phone, department, employment_status, academic_level)
    VALUES (?, ?, ?, ?, ?, ?)
    """, (name, email, phone, department, employment_status, academic_level))
    conn.commit()
    instructor_id = cursor.lastrowid
    conn.close()
    return instructor_id

def update_instructor(instructor_id, name, email, phone, department, employment_status, academic_level="Tertiary"):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("""
    UPDATE instructors
    SET name=?, email=?, phone=?, department=?, employment_status=?, academic_level=?
    WHERE id=?
    """, (name, email, phone, department, employment_status, academic_level, instructor_id))
    conn.commit()
    conn.close()

def set_instructor_academic_level(instructor_id, academic_level):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("""
    UPDATE instructors
...
def add_class_load(instructor_id, subject_name, section_code, room, day_of_week, start_time, end_time, type_, units, hours, load_category, enrollment=0):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("""
    INSERT INTO class_loads (instructor_id, subject_name, section_code, room, day_of_week, start_time, end_time, type, units, hours, load_category, enrollment)
